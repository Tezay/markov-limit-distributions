...
Considérons le chemin composite de $i$ à $i$ passant par $j$ : $i \xrightarrow{m} j \xrightarrow{n} j \xrightarrow{k} i$.
...
Considérons également le chemin de retour simple : $i \xrightarrow{m} j \xrightarrow{k} i$.
...
$$d(i) \mid [(m + n + k) - (m + k)] \implies d(i) \mid n$$
...
On peut échanger $d(j)$ et $d(i)$ et ainsi, on obtient : $d(j) \mid d(i) \quad (E4)$.
...
\subsubsection*{a) Prouver que $L = S$.}
...
Soit $\pi \in S$. $\pi P^k = \pi$ pour tout $k \geq 0$.
...
\subsubsection*{b) Propriétés de $L$ et $S$.}
...
\subsubsection*{c) Prouver que toute chaîne de Markov finie admet au moins une distribution stationnaire.}
...
1.  La matrice de transition $P$ est une \textbf{matrice de Markov}.
...
\subsubsection*{d) Montrer qu'une chaîne de Markov réductible admet une infinité de distributions limite.}
...
\subsubsection*{e) Montrer qu'une chaîne irréductible et $d$-périodique $(d \neq 1)$ n'admet pas toujours de distribution limite.}
...
\subsubsection*{f) Cherchez une preuve qu'il existe une unique distribution limite, indépendante de la distribution initiale, pour une chaîne finie, irréductible et apériodique.}

\noindent\textbf{Preuve Simplifiée :}
Une chaîne de Markov est dite \textbf{ergodique} si elle est à la fois \textbf{irréductible} et \textbf{apériodique}. Le théorème stipule que pour une chaîne ergodique, la distribution $\Pi(n)$ converge vers une distribution $\pi$ unique, indépendamment de la distribution initiale $\Pi(0)$.

\begin{enumerate}
    \item \textbf{Unicité de la Distribution Stationnaire ($\pi$)}
    \begin{itemize}
        \item L'existence de $\pi$ ($\pi = \pi P$) est toujours garantie (car $\lambda=1$ est une valeur propre de $P$).
        \item L'irréductibilité garantit que l'espace propre associé à $\lambda=1$ est de dimension 1. Le vecteur stationnaire $\pi$ (après normalisation) est donc unique.
    \end{itemize}

    \item \textbf{Convergence de la Matrice de Transition ($P^n$)}
    \begin{itemize}
        \item L'irréductibilité et l'apériodicité impliquent que $P$ est une \textbf{matrice primitive}.
        \item Le Théorème de Perron-Frobenius assure que la valeur propre dominante est $\lambda_1 = 1$ et que toutes les autres valeurs propres $\lambda_k$ ont $|\lambda_k| < 1$.
        \item Par conséquent, la matrice $P^n$ converge vers une matrice limite $W$ de rang 1 :
        $$
        \lim_{n\rightarrow+\infty} P^n = W
        $$
        \item Toutes les lignes de $W$ sont égales au vecteur stationnaire unique $\pi$.
    \end{itemize}

    \item \textbf{Indépendance de la Distribution Initiale}
    \begin{itemize}
        \item La limite de la distribution $\Pi(n)$ est donnée par :
        $$
        \lim_{n\rightarrow+\infty} \Pi(n) = \Pi(0) W
        $$
        \item En utilisant la propriété de $W$ (toutes les lignes sont $\pi$) et le fait que $\Pi(0)$ est un vecteur de probabilité ($\sum \Pi_i = 1$) :
        $$
        \Pi(0) W = \sum_{i} \Pi_i \cdot \pi = \left(\sum_{i} \Pi_i\right) \pi = 1 \cdot \pi = \pi
        $$
    \end{itemize}
\end{enumerate}
    \textbf{Conclusion :} La distribution limite est $\pi$, et elle est unique et indépendante de $\Pi(0)$.