...
Puisque les seules classes finales accessibles sont \(C_2\) et \(C_5\), nous aurons \(P(i \to C_5) = 1 - x_i\).
...
Pour les états de la classe \(C_7\) (\(10, 19, 22, 24\)), nous écrivons le système d'équations linéaires basé sur les transitions en un pas :
\[
\begin{cases}
...
\textbf{Résolution :}
En soustrayant l'équation (3) à l'équation (1), on obtient la relation :
...
En utilisant (2) et (4) et en substituant les variables, la résolution du système donne les valeurs exactes :
\[
x_{10} = \frac{2}{3}, \quad
x_{19} = \frac{1}{2}, \quad
x_{22} = \frac{1}{3}, \quad
...
Les probabilités d'atteindre \(C_5\) sont les compléments à 1 :
\[
P(10 \to C_5) = 1/3, \quad
P(19 \to C_5) = 1/2, \quad
P(22 \to C_5) = 2/3, \quad
...
./markov_analyzer --in data/moodle/matrix.txt --dist-start 10 --dist-steps 100 > data/q12/sim_10.txt
./markov_analyzer --in data/moodle/matrix.txt --dist-start 19 --dist-steps 100 > data/q12/sim_19.txt
./markov_analyzer --in data/moodle/matrix.txt --dist-start 22 --dist-steps 100 > data/q12/sim_22.txt
./markov_analyzer --in data/moodle/matrix.txt --dist-start 24 --dist-steps 100 > data/q12/sim_24.txt
\end{lstlisting}

Les résultats sont extraits de la ligne \texttt{[Distribution]} des fichiers de sortie, qui donne le vecteur \(\Pi^{(100)}\).
Pour obtenir la probabilité d'absorption dans une classe, nous sommons les probabilités des états appartenant à cette classe :
\begin{itemize}
