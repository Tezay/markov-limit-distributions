...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[xscale=3.5,yscale=8]
    ...
    \draw[->] (0,-0.02) -- (0,1.02) node[left=8pt] {$\Pi^{(n)}(i)$};
    ...
    \foreach \x/\lbl in {
        ...
        \draw[gray!30] (\x,0) -- (\x,1);
        \node[below] at (\x,0) {\lbl};
    }

    % Graduation sur les y de 0 à 1
